...
import { Maximize2, Archive, RotateCcw, CalendarPlus, Clock, Mail, Phone, FileText, UserPlus } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function LeadCard({ lead, isOverlay, disableDrag, onStatusChange, onSaveObs }: LeadCardProps) {
  ...
  if (isNovoLead) {
    ...
    switch (lead.situacao) {
      ...
      case 'Arquivado': default: theme.badgeBg = 'bg-slate-100'; theme.text = 'text-slate-500'; theme.border = 'border-slate-200'; theme.bg = 'bg-white'; break;
    }
  }

  const renderDetalhesDaPlanilha = () => {
    if ((lead as any).origem === 'Manual') {
      return (
        <div className="flex flex-col items-center justify-center p-6 text-center space-y-3 bg-indigo-50/30 rounded-lg border border-indigo-100 border-dashed">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1 max-w-[250px] mx-auto">
              Este lead foi cadastrado via Landing Page ou Painel e não possui os dados brutos de formulário do Meta Ads.
            </p>
          </div>
        </div>
      );
    ...
    const chavesOcultas = ['id', 'nome', 'whatsapp', 'cargo', 'tipo', 'situacao', 'email', 'responsavel', 'data_alteracao', 'observacoes', 'origem'];
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <div ref={setNodeRef} {...dragProps} className={`touch-none ${isOverlay ? 'cursor-grabbing rotate-2 scale-105 shadow-2xl opacity-90' : ''}`}>
          <Card className={`group relative mb-3 transition-all duration-300 text-left overflow-hidden ${cursorClass} ${theme.bg} ${theme.border} ${theme.ring} ${isNovoLead ? 'shadow-md' : 'shadow-sm'}`}>
            ...
            <div className="absolute top-2 right-2 flex items-center gap-1 z-20">
              <button
                type="button" title={lead.situacao === 'Arquivado' ? "Restaurar Lead" : "Arquivar Lead"}
                onClick={(e) => {
                  e.preventDefault(); e.stopPropagation();
                  onStatusChange?.(lead.id, lead.situacao === 'Arquivado' ? 'Novos Leads' : 'Arquivado');
                }}
                onPointerDown={(e) => e.stopPropagation()}
                className={`p-1.5 rounded-md transition-colors ${lead.situacao === 'Arquivado' ? 'text-indigo-400 hover:text-indigo-600 hover:bg-indigo-50' : 'text-slate-400 hover:text-rose-500 hover:bg-rose-50'}`}
              >
            ...
            <CardHeader className="p-4 pb-2 space-y-1 relative z-10 pr-14">
              {lead.cargo && <div className="text-[10px] font-bold text-indigo-600 uppercase tracking-wider leading-tight line-clamp-1">{lead.cargo}</div>}
              <CardTitle className="text-sm font-bold text-slate-800 leading-snug line-clamp-2">{lead.nome}</CardTitle>
            </CardHeader>

            <CardContent className="p-4 pt-0 space-y-3 relative z-10">
              <div className="space-y-1.5">
                {realPhone && <div className="flex items-center text-xs text-slate-600 truncate pr-4" title="Telefone/WhatsApp"><Phone className="w-3.5 h-3.5 mr-2 text-slate-400 shrink-0" /><span className="font-medium text-slate-700">{formatarTelefone(realPhone)}</span></div>}
                {realEmail && <div className="flex items-center text-xs text-slate-600 truncate pr-4" title="E-mail"><Mail className="w-3.5 h-3.5 mr-2 text-slate-400 shrink-0" /><span className="truncate">{realEmail}</span></div>}
              </div>
              ...
              <div className="flex flex-col gap-1.5 pt-3 border-t border-slate-100/60">
                ...
                <div className="flex items-center text-[10px] text-slate-500" title="Quando o lead entrou no sistema"><CalendarPlus className="w-3 h-3 text-slate-400 mr-1.5 shrink-0" /><span className="font-semibold text-slate-700 mr-1">Entrada:</span>{formatarData(lead.created_time)}</div>
                <div className="flex items-center text-[10px] text-slate-500" title="Última vez que o card mudou de coluna"><Clock className="w-3 h-3 text-slate-400 mr-1.5 shrink-0" /><span className="font-semibold text-slate-700 mr-1">Status alterado:</span>{formatarData(lead.data_alteracao)}</div>
              </div>
              ...
              <div className="flex items-center justify-center mt-3 pt-4 border-t border-slate-100/60 pb-1">
                <div className={`flex items-center justify-center w-full gap-2 px-4 py-2 rounded-lg shadow-sm ${theme.badgeBg}`}>
                  {isNovoLead && <span className="relative flex h-2.5 w-2.5 shrink-0"><span className={`animate-ping absolute inline-flex h-full w-full rounded-full ${theme.ping} opacity-75`}></span><span className={`relative inline-flex rounded-full h-2.5 w-2.5 ${theme.dot}`}></span></span>}
                  <span className={`text-xs font-extrabold uppercase tracking-widest ${theme.text}`}>{lead.situacao}</span>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <DialogContent className="max-w-md max-h-[85vh] overflow-y-auto custom-scrollbar">
        <DialogHeader>
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            ...
            {(lead as any).origem === 'Manual' && <Badge variant="default" className="bg-indigo-600 hover:bg-indigo-700">Adicionado Manualmente</Badge>}
          </div>
        </DialogHeader>
        ...
        <div className="mt-4 space-y-6">
          <div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-end">
                <Button onClick={handleSalvarObservacao} disabled={salvandoObs || obsTexto === (lead.observacoes || "")} className="bg-indigo-600 hover:bg-indigo-700 text-white shadow-sm" size="sm">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">Telefone</span><span className="text-sm font-medium text-slate-800">{realPhone ? formatarTelefone(realPhone) : '-'}</span></div>
              <div><span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block mb-1">E-mail</span><span className="text-sm font-medium text-slate-800 break-words">{realEmail || '-'}</span></div>
            </div>
          </div>

          <div>
            <h4 className="font-semibold text-slate-900 border-b border-slate-200 pb-2 mb-3">Dados da Origem</h4>
