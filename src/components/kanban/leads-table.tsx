...
function serializarFiltros(filters: Record<string, ColFilter>): Record<string, ColFilterRaw> {
  ...
  for (const [colId, f] of Object.entries(filters)) {
    if (f.selectedValues.size > 0)
      result[colId] = { selectedValues: Array.from(f.selectedValues), invertMode: f.invertMode };
  }
  return result;
}
...
function formatarTelefone(raw: string): string {
  ...
  if (nums.startsWith("55") && nums.length >= 12) {
    const ddd = nums.slice(2, 4), rest = nums.slice(4);
...
function getCellValue(lead: Lead, colId: string): string {
  if (colId === "email")    { const { realEmail } = resolverContatos(lead); return realEmail  || lead.email    || ""; }
  if (colId === "whatsapp") { const { realPhone } = resolverContatos(lead); return realPhone  || lead.whatsapp || ""; }
...
export function LeadsTable({ leads, onStatusChange, onSaveObs }: LeadsTableProps) {
  ...
  const handleSort = (colId: string) => {
    let newCol: string | null = colId, newDir: SortDir = "asc";
  ...
  const limparTodosFiltros = () => { setColFilters({}); lsSet(LS_FILTROS, {}); };
  ...
  const hasActiveFilter = (colId: string) => {
    const f = colFilters[colId]; return f && f.selectedValues.size > 0;
  };
  ...
  const uniqueValues = useMemo(() => {
    ...
    for (const col of COLUNAS_DISPONIVEIS) {
      map[col.id] = Array.from(new Set(leads.map(l => getCellValue(l, col.id) || "(vazio)")))
  ...
  const leadsProcessados = useMemo(() => {
    ...
    for (const [colId, f] of Object.entries(colFilters)) {
      ...
      result = result.filter(lead => {
        ...
        return f.invertMode ? !f.selectedValues.has(val) : f.selectedValues.has(val);
      });
    }
    if (sortCol && sortDir) {
      result.sort((a, b) => {
  ...
  return (
    <>
          ...
          <Dialog>
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <div className="border-t border-slate-100 pt-3 flex justify-end">
                <button onClick={restaurarColunasPadrao} className="text-xs text-slate-500 hover:text-indigo-600 font-medium transition-colors">
                  Restaurar padrão
                </button>
              </div>
            </DialogContent>
                    ...
                    <th key={col.id} className="text-left border-b border-slate-200 bg-slate-50 select-none" style={{ padding: 0 }}>
                      ...
                      {openFilterCol === col.id && (
                        <ColumnFilterDropdown
                          colId={col.id} colLabel={col.label}
              ...
              {leadsProcessados.length === 0 ? (
                ...
                leadsProcessados.map(lead => (
                  <tr
                    ...
                    className="hover:bg-indigo-50/40 transition-colors border-b border-slate-100 last:border-0 cursor-pointer group"
                  >
                    {colunasAtivas.map(col => {
                      ...
                      return (
                        <td key={col.id} className="px-3 py-3 text-slate-600 whitespace-nowrap max-w-[300px] overflow-hidden text-ellipsis">
                          {col.id === "situacao" ? (
                            ...
                            <span className={col.id === "nome" ? "font-medium text-slate-900 group-hover:text-indigo-700 transition-colors" : ""}>
      ...
      {/* Modal de detalhe — compartilhado com o LeadCard */}
      {leadSelecionado && (
        <LeadDetailModal
          lead={leadSelecionado}
          open={!!leadSelecionado}
          onOpenChange={(open) => { if (!open) setLeadSelecionado(null); }}
...
const SITUACAO_COLORS: Record<string, string> = {
  ...
  "Reuni��o agendada":    "bg-purple-50 text-purple-700 border-purple-200",
...
interface DropdownProps {
  colId: string; colLabel: string; allValues: string[];
  filter: ColFilter; onFilterChange: (p: Partial<ColFilter>) => void;
  ...
  return (
    <div className="absolute z-50 mt-1 w-72 bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden" onClick={e => e.stopPropagation()}>
      ...
      <div className="px-3 py-2 border-b border-slate-100 flex items-center gap-2">
        ...
        <button onClick={() => onFilterChange({ invertMode: false })} className={`px-2.5 py-1 rounded text-xs font-semibold transition-colors ${!filter.invertMode ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>Mostrar selecionados</button>
        <button onClick={() => onFilterChange({ invertMode: true })} className={`px-2.5 py-1 rounded text-xs font-semibold transition-colors ${filter.invertMode ? "bg-red-500 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}>Ocultar selecionados</button>
      </div>
      <div className="px-3 py-2 border-b border-slate-100">
        <div className="relative">
          <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400" />
          <input type="text" placeholder="Buscar valor..." value={localSearch} onChange={e => setLocalSearch(e.target.value)}
            className="w-full pl-8 pr-3 py-1.5 text-xs border border-slate-200 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-400 bg-slate-50" autoFocus />
        </div>
      </div>
        ...
        {visibleValues.length === 0
          ...
          : visibleValues.map(val => (
              <label key={val} className="flex items-center gap-2.5 px-3 py-1.5 hover:bg-slate-50 cursor-pointer transition-colors">
                <input type="checkbox" checked={filter.selectedValues.has(val)} onChange={() => toggleValue(val)}
      ...
      <div className="px-3 py-2 border-t border-slate-100 flex items-center justify-between bg-slate-50">
        ...
        <div className="flex gap-2">
          {someSelected && <button onClick={onClear} className="text-xs text-slate-500 hover:text-red-500 font-medium transition-colors">Limpar</button>}
          <button onClick={onClose} className="text-xs px-3 py-1 bg-indigo-600 text-white rounded-md font-semibold hover:bg-indigo-700 transition-colors">Aplicar</button>
        </div>
      </div>
    </div>
  );
